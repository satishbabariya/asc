// std/crypto/sha384.ts — Pure SHA-384 implementation (RFC-0018)
// SHA-384 is SHA-512 with different initial hash values and output truncated to 48 bytes.
...
/// SHA-512/384 round constants (same as SHA-512: first 64 bits of cube roots of first 80 primes).
const K: [u64; 80] = [
...
impl Sha384 {
  ...
  /// Finalize and return the 48-byte hash (first 6 state words).
  fn finalize(refmut<Self>): [u8; 48] {
    ...
    // Produce 48-byte output (first 6 of 8 state words).
    let hash: [u8; 48] = [0u8; 48];
    let i: usize = 0;
    while i < 6 {
      let w = self.state[i];
      hash[i * 8] = (w >> 56) as u8;
...
/// One-shot SHA-384: hash input and return 48-byte digest.
